...
void EspidfBleKeyboard::send_string(const std::string &str) {
    ...
    if (str == last_send_string_ && (now - last_send_string_ms_) < 30) {
        ESP_LOGD(TAG, "send_string dedup: \"%s\" (duplicate after %lums)", str.c_str(), now - last_send_string_ms_);
        return;
    }
    ...
    while (i < str.size()) {
        ...
        if (m.keycode != 0x00) {
            ...
            ESP_LOGD(TAG, "send_string: skipped unmapped codepoint U+%04lX", cp);
        }
    }
...
void EspidfBleKeyboard::send_key_combo(uint8_t modifiers, uint8_t keycode) {
    ...
    if (key_id == last_send_key_id_ && (now - last_send_key_ms_) < 30) {
        ESP_LOGD(TAG, "send_key_combo dedup: mod=0x%02X key=0x%02X (duplicate after %lums)",
...
void EspidfBleKeyboard::send_consumer(uint16_t usage) {
    ...
    if (usage == last_consumer_usage_ && (now - last_consumer_ms_) < 30) {
        ESP_LOGD(TAG, "send_consumer dedup: 0x%04X (duplicate after %lums)", usage, now - last_consumer_ms_);
        return;
    }
...
void EspidfBleKeyboard::send_mouse_click(uint8_t buttons) {
    ...
    if (buttons == last_mouse_click_ && (now - last_mouse_click_ms_) < 30) {
        ESP_LOGD(TAG, "send_mouse_click dedup: 0x%02X (duplicate after %lums)", buttons, now - last_mouse_click_ms_);
        return;
    }

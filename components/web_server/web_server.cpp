...
esp_err_t WebServer::api_params_get_handler(httpd_req_t *req) {
    ...
    if (buf_len > 1) {
        ...
        if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
            ...
            auto parseParam = [&buf](const char* param_name, auto&& extractParam){
                char param_value[32]{};
            ...
            if (!fail) {
                Params params{.temp=temp, .time=time};
                ...
                if (xQueueOverwrite(params_queue, &params) == pdPASS) {
                    ESP_LOGI("HTTPD_params", "Params temp=%d, time=%d queued", params.temp, params.time);
                    char resp[100];
                    sprintf(resp, "Stored params: temp=%d, time=%d\n", params.temp, params.time);
...
esp_err_t WebServer::api_status_get_handler(httpd_req_t *req) {
    ...
    if (xQueuePeek(status_queue, &last_status, 0) == pdTRUE) {
        ...
        cJSON_AddNumberToObject(root, "temp_celsius", last_status.temp_celsius);
        ...
        cJSON_AddBoolToObject(root, "fault_lockout", last_status.fault_lockout);
...
void WebServer::start() {
    ...
    httpd_uri_t uris[] = {
        ...
        {"/api/params", HTTP_GET, [](httpd_req_t *req) -> esp_err_t {
            WebServer* instance = static_cast<WebServer*>(req->user_ctx);
        ...
        {"/ws", HTTP_GET, [](httpd_req_t *req) -> esp_err_t {
            WebServer* instance = static_cast<WebServer*>(req->user_ctx);
